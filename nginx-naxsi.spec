...
Source1: src/logrotate
Source2: src/nginx.init
Source3: src/nginx.sysconf
Source4: src/nginx.conf
Source5: src/nginx.vh.default.conf
Source8: src/syslog_1.5.2.patch
...
a mail proxy server
This version comes with HttpLimitConnModule, HttpLimitReqModule,
 syslog and Naxsi
For syslog see https://github.com/yaoweibin/nginx_syslog_patch for usage
...
tar -xzf $RPM_SOURCE_DIR/naxsi-core-0.49.tgz
echo "toto"

tar -xzf $RPM_SOURCE_DIR/nginx-1.5.2.tar.gz
patch -p0 < $RPM_SOURCE_DIR/syslog_1.5.2.patch
#%setup -q

%build
mv nginx-1.5.2 nginx-naxsi-1.5.2
cd nginx-naxsi-1.5.2
./configure \
        ...
        --with-file-aio \
        --with-ipv6 \
        --add-module=%{_builddir}/naxsi-core-0.49/naxsi_src \
        --with-debug \
	--with-syslog \
...
# install naxsi stuff
%{__install} -m 644 -p %{_builddir}/naxsi-core-0.49/naxsi_config/naxsi_core.rules \
...
%{__mkdir} -p $RPM_BUILD_ROOT/usr/local/share/selinux
%clean
%{__rm} -rf $RPM_BUILD_ROOT
...
#/usr/local/share/selinux/nginx.fc
